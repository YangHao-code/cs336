...
def run_multihead_self_attention(
    ...
    """
    model = MultiAtt(d_model, num_heads, attn_pdrop)
    d_kq = d_model // num_heads
    for i in range(num_heads):
        qname = f"q_heads.{i}.weight"
        kname = f"k_heads.{i}.weight"
        vname = f"v_heads.{i}.weight"
        model.qkv_projection.weight.data[i * d_kq:(i + 1) * d_kq] = weights[qname]
        model.qkv_projection.weight.data[(num_heads + i)* d_kq:(num_heads + i + 1) * d_kq] = weights[kname]
        model.qkv_projection.weight.data[(num_heads * 2 + i)* d_kq:(num_heads * 2 + i + 1) * d_kq] = weights[vname]
    model.output_proj.weight.data = weights["output_proj.weight"]
    ans = model(in_features)
    return ans
...
def run_transformer_block(
    ...
    """
    model = trans_block(d_model, num_heads, d_ff, attn_pdrop, residual_pdrop)
    d_kq = d_model // num_heads
    model.multiatt_block.qkv_projection.weight.data[:num_heads * d_kq] = weights["attn.q_proj.weight"]
    model.multiatt_block.qkv_projection.weight.data[num_heads * d_kq:num_heads * d_kq * 2] = weights["attn.k_proj.weight"]
    model.multiatt_block.qkv_projection.weight.data[num_heads * d_kq * 2:num_heads * d_kq * 3] = weights["attn.v_proj.weight"]
    model.multiatt_block.output_proj.weight.data = weights["attn.output_proj.weight"]
    model.ffn_block.w1.weight.data = weights["ffn.w1.weight"]
    model.ffn_block.w2.weight.data = weights["ffn.w2.weight"]
    model.norm_block1.weight.data = weights["ln1.weight"]
    model.norm_block2.weight.data = weights["ln2.weight"]
    ans = model(in_features)
    return ans
...
def run_transformer_lm(
    ...
    model.token_emb_block.weight.data = weights["token_embeddings.weight"]
    model.pos_emb_block.weight.data = weights["position_embeddings.weight"]
    for i in range(num_layers):
        model.trans_blocks[i].multiatt_block.qkv_projection.weight.data[:num_heads * d_kq] = weights[f"layers.{i}.attn.q_proj.weight"]
        model.trans_blocks[i].multiatt_block.qkv_projection.weight.data[num_heads * d_kq:num_heads * d_kq * 2] = weights[f"layers.{i}.attn.k_proj.weight"]
        model.trans_blocks[i].multiatt_block.qkv_projection.weight.data[num_heads * d_kq * 2:num_heads * d_kq * 3] = weights[f"layers.{i}.attn.v_proj.weight"]
        model.trans_blocks[i].multiatt_block.output_proj.weight.data = weights[f"layers.{i}.attn.output_proj.weight"]
        model.trans_blocks[i].ffn_block.w1.weight.data = weights[f"layers.{i}.ffn.w1.weight"]
        model.trans_blocks[i].ffn_block.w2.weight.data = weights[f"layers.{i}.ffn.w2.weight"]
        model.trans_blocks[i].norm_block1.weight.data = weights[f"layers.{i}.ln1.weight"]
        model.trans_blocks[i].norm_block2.weight.data = weights[f"layers.{i}.ln2.weight"]
    # 确认权重形状是否匹配
    assert model.trans_blocks[i].ffn_block.w1.weight.shape == weights[f"layers.{i}.ffn.w1.weight"].shape
    model.out_norm_block.weight.data = weights["ln_final.weight"]
    model.out_linear_block.weight.data = weights["lm_head.weight"]
    ans = model(in_indices)
    return ans
...
def run_rmsnorm(
    ...
    model.weight.data = weights["weight"]
    ans = model(in_features)
    return ans
